...
// project import
// ==============================|| React - redux ||============================== //
import auth, { login } from '@/redux/features/auth-slice'
import loader from '@/redux/features/loader-slice'
import { getStatusAccount } from '@/redux/selectors'
...
const Login = (props) => {
    ...
    useEffect(() => {
        ...
        // dispatch(loader.actions.setLoaderOn(user.status === 'loading'))
        if (user.EC === 0) {
            console.log('login success')
            // setShowSnackbar(false)
            router.push('/')
        } else if (user.EC !== 0 && user.status !== 'logout' && user.EM !== '') {
            setMessage(user.EM)
